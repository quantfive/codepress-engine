...
function encode(relPath) {
  if (!relPath) return "";
  const xored = Buffer.from(relPath).map((b, i) => b ^ SECRET[i % SECRET.length]);
  return xored.toString("base64") // 1.2× length of path
  .replace(/[+/=]/g, c => ({
    "+": "-",
    "/": "_",
    "=": ""
...
function decode(attr) {
  if (!attr) return "";
  const buf = Buffer.from(attr.replace(/[-_]/g, c => ({
    "-": "+",
    _: "/"
  })[c]), "base64");
...
function detectGitBranch() {
  const fromEnv = process.env.GIT_BRANCH ||
  // Vercel
  ...
  try {
    ...
    console.log("\x1b[33m⚠ Could not detect git branch, using default: main\x1b[0m");
    return "main";
  }
}
  ...
  return {
    ...
    visitor: {
      ...
      JSXOpeningElement(nodePath, state) {
        ...
        const endLine = nodePath.parent.loc ? nodePath.parent.loc.end.line : startLine;
        const currentAttributeValue = `${encodedPath}:${startLine}-${endLine}`;
        let existingAttribute = node.attributes.find(attr => t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name, {
          name: attributeName
        }));
        if (existingAttribute) {
          ...
          // Add new attribute
          node.attributes.push(t.jsxAttribute(t.jsxIdentifier(attributeName), t.stringLiteral(currentAttributeValue)));
        }
        ...
        if (repoName && !globalAttributesAdded) {
          ...
          if (isSuitableElement) {
            ...
            const hasRepoAttribute = node.attributes.some(attr => t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name, {
              name: repoAttributeName
            }));
            if (!hasRepoAttribute) {
              console.log(`\x1b[32m✓ Adding repo attribute globally to <${elementName}> in ${path.basename(state.file.opts.filename)}\x1b[0m`);
            ...
            const hasBranchAttribute = node.attributes.some(attr => t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name, {
              name: branchAttributeName
            }));
            if (!hasBranchAttribute && branch) {
              console.log(`\x1b[32m✓ Adding branch attribute globally to <${elementName}> in ${path.basename(state.file.opts.filename)}\x1b[0m`);
