...
function acquireLock() {
  try {
    ...
    const lockData = fs.existsSync(lockPath) ? JSON.parse(fs.readFileSync(lockPath, "utf8")) : null;
    if (lockData) {
...
function applyTextChanges(fileContent, changes) {
  ...
  for (const change of sortedChanges) {
    ...
    switch (type) {
      case "replace":
        if (startIdx !== undefined && endIdx !== undefined && change.codeChange !== undefined) {
  ...
  changes.forEach((change, index) => {
    if (change.find) {
      if (findPatterns.has(change.find)) {
        ...
        console.warn(`\x1b[33m⚠ CONFLICT DETECTED: Multiple changes target the same pattern\x1b[0m`);
        console.warn(`  Change ${existingIndex + 1}: ${changes[existingIndex].explanation}`);
        console.warn(`  Change ${index + 1}: ${change.explanation}`);
        console.warn(`  Pattern: ${change.find.substring(0, 100)}...`);
        console.warn(`\x1b[33m  → Only the first change will be applied\x1b[0m`);
      } else {
        ...
        if (find && replaceWith !== undefined) {
          if (modifiedContent.includes(find)) {
            ...
            if (openTags !== closeTags) {
              console.warn(`\x1b[33m⚠ POTENTIAL JSX MALFORMATION: Unmatched tags in replacement\x1b[0m`);
              console.warn(`  Open tags: ${openTags}, Close tags: ${closeTags}`);
              console.warn(`  Replacement: ${replaceWith.substring(0, 200)}...`);
            }
            ...
            console.warn(`\x1b[33m  This might be due to a previous change modifying the content\x1b[0m`);
          }
        } else {
          console.warn("Invalid 'replace' change object:", change);
        }
...
function readFileFromEncodedLocation(encodedLocation) {
  ...
  const targetFile = path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath);
...
async function applyChangesAndFormat(fileContent, changes, targetFile, usePatternChanges = true) {
  ...
  const modifiedContent = usePatternChanges ? applyPatternChanges(fileContent, changes) : applyTextChanges(fileContent, changes);
  ...
  console.log(`\x1b[32m✓ Updated file ${targetFile} with ${changes.length} changes using ${usePatternChanges ? "pattern-based" : "text-based"} approach\x1b[0m`);
...
async function getAiChanges({
  ...
  console.log(`\x1b[36mℹ Getting AI changes from backend for file encoded_location: ${encodedLocation}\x1b[0m`);
  ...
  return await callBackendApi("POST", "code-sync/get-ai-changes", {
...
async function getAgentChanges({
  ...
  console.log(`\x1b[36mℹ Getting agent changes from backend for file encoded_location: ${encodedLocation}\x1b[0m`);
  return await callBackendApi("POST", "code-sync/get-agent-changes", {
    old_html: oldHtml,
    new_html: newHtml,
    github_repo_name: githubRepoName,
    encoded_location: encodedLocation,
...
function createApp() {
    ...
    try {
      ...
      for (const change of changes) {
        ...
        try {
          ...
          if (!hasStyleChanges && !hasTextChanges) {
            console.warn(`\x1b[33m⚠ Skipping change with no style or text changes.\x1b[0m`);
            continue;
          }
          ...
          console.error(`\x1b[31m✖ Error processing change for location: ${change.encoded_location}\x1b[0m`);
        }
      }
      ...
      for (const encodedLocation of uniqueEncodedLocations) {
        try {
          ...
          console.error(`\x1b[31m✖ Error reading file for location: ${encodedLocation}\x1b[0m`);
        }
      }
      ...
      for (const change of validChanges) {
        try {
          ...
          fileChangesForBackend.push({
            ...
            // Preserve individual encoded_location
            file_content: fileContent,
            changes: [{
      ...
      if (backendResponse && backendResponse.updated_files) {
        ...
        for (const [filePath, newContent] of Object.entries(backendResponse.updated_files)) {
          const targetFile = filePath.startsWith("/") ? filePath : path.join(process.cwd(), filePath);
  ...
  app.post("/visual-editor-api-agent", async (request, reply) => {
    try {
      ...
      // Debug: Log auth header info
      console.log(`\x1b[36mℹ [visual-editor-api-agent] Auth header received: ${authHeader ? "[PRESENT]" : "[MISSING]"}\x1b[0m`);
      const {
        targetFile,
        fileContent
      ...
      const formattedCode = await applyChangesAndFormat(fileContent, backendResponse.changes, targetFile);
      return reply.code(200).send({
        success: true,
  ...
  app.post("/visual-editor-api-ai", async (request, reply) => {
    try {
      ...
      // Debug: Log auth header info
      console.log(`\x1b[36mℹ [visual-editor-api-ai] Auth header received: ${authHeader ? "[PRESENT]" : "[MISSING]"}\x1b[0m`);

      // Debug logging to see what's being received
      console.log(`\x1b[36mℹ Visual Editor AI API Request data: ${JSON.stringify({
        encoded_location,
      ...
      try {
        ...
        if (backendResponse.updated_files) {
          ...
          for (const [filePath, newContent] of Object.entries(backendResponse.updated_files)) {
            ...
            const targetFilePath = filePath.startsWith("/") ? filePath : path.join(process.cwd(), filePath);
            ...
            console.log(`\x1b[32m✓ Updated file ${filePath} with complete AI-generated content\x1b[0m`);
            results.push({
              path: filePath,
              modified_content: formattedCode
            });
        ...
        for (const fileData of responseData) {
          ...
          // Determine the target file path
          const targetFilePath = fileData.path.startsWith("/") ? fileData.path : path.join(process.cwd(), fileData.path);
          ...
          const formattedCode = await applyChangesAndFormat(currentFileContent, fileData.changes, targetFilePath);
...
function getProjectStructure() {
  try {
    ...
    let excludePatterns = [/^\.git(\/.*)?$/ // Exclude .git directory by default
    ];
    if (fs.existsSync(gitignorePath)) {
      const gitignoreContent = fs.readFileSync(gitignorePath, "utf8");
      const gitignorePatterns = gitignoreContent.split("\n").map(line => line.trim()).filter(line => line && !line.startsWith("#")) // Remove empty lines and comments
      .map(pattern => {
        ...
        regexPattern = regexPattern.replace(/\./g, "\\.") // Escape dots
        ...
        .replace(/\}/g, "\\}").replace(/\|/g, "\\|"); // Escape pipe

        // Convert gitignore wildcards to regex
        regexPattern = regexPattern.replace(/\*\*/g, ".*") // ** matches any number of directories
