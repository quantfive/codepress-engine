...
function wrapWithProvider(source: string): string {
  ...
  const componentName =
    defaultExportMatch?.[1] || source.match(/export\s+default\s+(\w+);/)?.[1];
...
export function createCodePressPlugin(
      ...
      build.onLoad(
        ...
        async (args: OnLoadArgs): Promise<OnLoadResult | null> => {
          try {
            ...
            let transformed = injectJSXAttributes(
              source,
              encoded,
              repo_name,
              branch_name
            ...
            console.error(
              "[CodePress Plugin] Error transforming",
              args.path,
              err
            );
            return null;
