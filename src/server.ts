/**
 * @fileoverview Codepress Development Server (TypeScript)
...
} from "./services/textChangeService";
import {
  getChanges,
  getAgentChanges,
...
function acquireLock(): boolean {
  try {
    const lockPath: string = path.join(os.tmpdir(), "codepress-dev-server.lock");

    // Try to read the lock file to check if the server is already running
    const lockData: LockData | null = fs.existsSync(lockPath)
...
function getProjectStructure(): string {
  try {
    ...
    if (fs.existsSync(gitignorePath)) {
      ...
      const gitignorePatterns: RegExp[] = gitignoreContent
        .split("\n")
        ...
        .map((pattern: string): RegExp | null => {
          ...
          let regexPattern: string = pattern;
        ...
        .filter((regex): regex is RegExp => regex !== null); // Remove null entries
    ...
    function shouldExclude(relativePath: string): boolean {
      return excludePatterns.some((pattern: RegExp) => pattern.test(relativePath));
    }
    ...
    function getFilesRecursively(dir: string, baseDir: string = dir): string[] {
      ...
      try {
        ...
        logger.warn(`Error reading directory ${dir}`, { error: (error as Error).message });
      }

      return files;
    }

    const fileList: string[] = getFilesRecursively(process.cwd());
    ...
    logger.error("Error generating project structure", { error: (error as Error).message });
...
function createApp(): FastifyInstance {
  ...
  app.get("/project-structure", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const structure: string = getProjectStructure();
      return reply.code(200).send({
        success: true,
        structure: structure,
      });
    } catch (error) {
      logger.error("Error getting project structure", { error: (error as Error).message });
  ...
  app.post("/visual-editor-api", async (request: FastifyRequest<{ Body: VisualEditorRequest }>, reply: FastifyReply) => {
    ...
    try {
      ...
      const changesWithDimensions: ChangeRequest[] = changes.filter(
        (change: ChangeRequest) => change.browser_width && change.browser_height
      );
      if (changesWithDimensions.length > 0) {
        const sampleChange: ChangeRequest = changesWithDimensions[0];
      ...
      const validChanges: ChangeRequest[] = [];
      ...
      for (const change of changes) {
        ...
        try {
          ...
          const encodedFilePath: string = change.encoded_location.split(":")[0];
          const targetFile: string = decode(encodedFilePath);
          if (!targetFile) {
      ...
      const fileChangesForBackend: any[] = [];
      for (const change of validChanges) {
        try {
          // Get pre-fetched file content from map
          const fileContent: string | undefined = fileContentMap.get(change.encoded_location);
          if (!fileContent) {
      ...
      if (backendResponse && backendResponse.updated_files) {
        ...
        for (const [filePath, newContent] of Object.entries(
          ...
          const targetFile: string = toAbsolutePath(filePath);
  ...
  app.post("/visual-editor-api-agent", async (request: FastifyRequest<{ Body: AgentRequest }>, reply: FastifyReply) => {
    const startTime: number = Date.now();
    try {
      ...
      if (backendResponse && backendResponse.updated_files) {
        const results: Array<{ path: string; modified_content: string }> = [];
        for (const [filePath, newContent] of Object.entries(
          backendResponse.updated_files
        )) {
          const targetFilePath: string = toAbsolutePath(filePath);
          const formattedCode: string = await applyFullFileReplacement(
            newContent,
            targetFilePath
          );
      ...
      if (backendResponse && backendResponse.modified_content !== null) {
        const modifiedContentStr: string = String(backendResponse.modified_content);
        const formattedCode: string = await applyFullFileReplacement(
          modifiedContentStr,
          targetFile
        );
      ...
      if (
        ...
        const currentContent: string = fs.readFileSync(targetFile, "utf8");
        const formattedCode: string = await applyChangesAndFormat(
          currentContent,
          fileData.changes,
          targetFile,
          false
 ...
 */
export async function startServer(options: ServerOptions = {}): Promise<FastifyInstance | null> {
  ...
  // Get the fixed port
  const port: number = options.port || getServerPort();
  ...
  try {
    ...
    if ((err as any).code === "EADDRINUSE") {
      ...
      logger.error("Codepress Dev Server error", { error: (err as Error).message });
    }
    return null;
  }
}
...
if (process.env.NODE_ENV !== "production") {
  ...
  (async () => {
    try {
      (serverModule as any).server = await startServer();
    } catch (err) {
      logger.error("Failed to auto-start server", { error: (err as Error).message });
    }
  })();
}
